...
**Import the data**
```{r step1}
global_temps <- data.frame(read.csv("GlobalTemp.csv"))
...
summary_temp_data <-  global_temps %>% group_by(Year, Country, Region) %>% 
summarize(mean(AverageTemperature))
...
forest_area <- data.frame(read_xls("forest_area.xls"))
...
renewable_energy_pct <- data.frame(read_xls("renewable_energy.xls"))
...
population <- data.frame(read_xls("population.xls"))
...
energy_use <- data.frame(read_xls("energy_use.xls"))
...
colnames(summary_temp_data) <- c("Year", "Country", "Region", "Temperature")
...
temp_data_cor <- subset(temp_data, select=-c(1,2,8))
tempcor <- cor(temp_data_cor)
tempcor
corrplot(tempcor, method = "number", type="upper")

#linear model
model1 <- lm(Temperature ~ Forest_Area + Renew_Energy_Pct + Population_Total + Energy_Use + Co2_Emissions, data=temp_data)
summary(model1)
```
