...
**Import the data**
```{r step01}
data <- data.frame(read_csv("/Users/babayegar/Downloads/DATS6101/city_temperature.csv"))
data
```
...
**Import the data**
```{r step02}
specified_Year =  subset(data,data$Year >= 1880)
year_temp_data <-  specified_Year %>% group_by(Region, Year) %>% 
summarize(mean(AvgTemperature))
year_temp_data_line_plot = ggplot() + 
  geom_line(data = year_temp_data, aes(x = Year, y = `mean(AvgTemperature)`, group = Region,color=Region)) +
  xlab('Year') +
  ylab('Temperature Level') 
year_temp_data_qq_plot <- ggplot(year_temp_data, aes(sample = `mean(AvgTemperature)`, colour = Region)) +
  stat_qq() +
  stat_qq_line()
